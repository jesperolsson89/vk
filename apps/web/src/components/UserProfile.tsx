  ...
  return (
    <div className="flex flex-col gap-4 mt-6 mb-20">
      ...
      <div className="flex flex-col gap-3 font-semibold px-6 py-3 rounded-lg transition-colors duration-200 shadow-md bg-primary-100">
        <h2>Dina viktigaste värderingar just nu:</h2>
        {latest ? (
          <ol className="list-decimal list-inside flex flex-col gap-1 font-normal">
            {(latest.topValues as string[]).map((value, i) => (
              <li key={i} className="text-text">{value}</li>
            ))}
          </ol>
        ) : (
          <p className="font-normal text-text-muted">Inga resultat sparade än.</p>
        )}
      </div>
      ...
      <div className="flex flex-col gap-3 font-semibold px-6 py-3 rounded-lg transition-colors duration-200 shadow-md bg-primary-100">
        ...
        {results.length === 0 ? (
          ...
          results.map((result) => (
            <div key={result.id} className="flex flex-col gap-1 font-normal border-t border-border pt-2">
